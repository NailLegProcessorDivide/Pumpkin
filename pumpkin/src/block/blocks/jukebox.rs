...
impl JukeboxBlock {
    fn has_record(&self, block: &Block, location: &BlockPos, world: &World) -> bool {
        let state_id = world.get_block_state(location).id;
        JukeboxLikeProperties::from_state_id(state_id, block).has_record
    }

    fn set_record(&self, has_record: bool, block: &Block, location: &BlockPos, world: &Arc<World>) {
        let new_state = JukeboxLikeProperties { has_record };
        world.set_block_state(location, new_state.to_state_id(block), BlockFlags::empty());
    }

    fn stop_music(&self, block: &Block, position: &BlockPos, world: &Arc<World>) {
        self.set_record(false, block, position, world);
        world.sync_world_event(WorldEvent::JukeboxStopsPlaying, *position, 0);
    }
}
...
impl BlockBehaviour for JukeboxBlock {
    fn use_with_item(&self, args: UseWithItemArgs<'_>) -> BlockActionResult {
        ...
        if self.has_record(args.block, args.position, world) {
            self.stop_music(args.block, args.position, world);
